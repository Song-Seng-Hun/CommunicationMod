...
const REVIEWED_CONTRACT='2026-09-18-character-mechanics-v8',digest=v=>createHash('sha256').update(v).digest('hex'),object=v=>v!==null&&typeof v==='object'&&!Array.isArray(v);
...
export function validateCatalog(capabilities){
 const names=new Set(),ids=new Set(),report=[];for(const cap of capabilities){assert.match(cap.id,/^[a-z][a-z0-9.-]*$/);assert.ok(!names.has(cap.id),'duplicate capability');names.add(cap.id);assert.deepEqual(cap.cases.map(c=>c.case).sort(),['exception','incomplete','normal']);assert.ok(cap.covers.length>0,cap.id+' coverage');for(const c of cap.cases){assert.equal(c.id,cap.id+'/'+c.case);assert.equal(c.capability,cap.id);assert.ok(c.revision);assert.ok(!ids.has(c.id),'duplicate example');ids.add(c.id);for(const key of ['when','not_when','requires','expect','stop'])assert.ok(typeof c[key]==='string'&&c[key].trim(),c.id+': '+key);assert.ok(object(c.bindings));assert.ok(Array.isArray(c.calls));const visible=publicCapsule(c),fragment={ref:'guidance/'+c.id,data:visible};assert.ok(JSON.stringify(fragment).length<=2400,c.id+': exceeds 2400 character public budget ('+JSON.stringify(fragment).length+')');const values=samples(c.bindings);for(const call of c.calls){const args=publicArguments(call.tool,call.arguments);for(const key of ['action_id','request_id','refs'])if(Object.hasOwn(args,key)){const v=args[key];assert.ok(object(v)&&Object.hasOwn(v,'$bind')||Array.isArray(v)&&v.every(x=>object(x)&&Object.hasOwn(x,'$bind')),c.id+': literal dynamic '+key);}bindCall(call,c.bindings,values);}}report.push({id:cap.id,examples:cap.cases.length,covers:cap.covers});}return {capabilities:report,examples:ids.size};
}
