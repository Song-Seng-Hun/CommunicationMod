export const obj = (x) => x !== null && typeof x === 'object' && !Array.isArray(x) ? x : {};
...
function compact(x) {
    if (Array.isArray(x))
        return x.map(compact);
    if (x && typeof x === 'object')
        return Object.fromEntries(Object.entries(obj(x)).filter(([k]) => !['description_source', 'description_rendering', 'tooltips_source', 'tooltips_rendering', 'render_frame'].includes(k)).map(([k, v]) => [k, compact(v)]));
    return x;
}
function mechanicSummary(value) {
    const out = { ...obj(value) };
    for (const key of ['collection', 'combat_collection'])
        if (out[key]) {
            const pile = { ...obj(out[key]) };
            delete pile.cards;
            pile.cards_context = key;
            out[key] = pile;
        }
    return out;
}
export function focus(state) {
    ...
    const out = { ...pick(state, ['session_id', 'state_id', 'ready', 'support', 'runtime', 'connection']),
    ...
    if (game.mechanics)
        obj(out.player).mechanics = mechanicSummary(game.mechanics);
    if (Object.keys(obj(game.map_plan)).length)
        out.map_plan = pick(obj(game.map_plan), planKeys);
    ...
    if (Object.keys(combat).length) {
        const c = { ...combat };
        if (obj(combat.player).mechanics)
            c.player = { ...obj(combat.player), mechanics: mechanicSummary(obj(combat.player).mechanics) };
        for (const pile of ['draw_pile', 'discard_pile', 'exhaust_pile']) {
            c[pile + '_count'] = list(c[pile]).length;
            delete c[pile];
        }
        if (combat.hand_complete === false)
            c.hand = [];
        out.combat = c;
    }
    ...
    if (screen.event_reading) {
        ...
        if (previews.length)
            screen.option_card_previews = previews;
        delete reading.last_discussion;
        delete screen.body_text;
        delete screen.options;
        screen.event_reading = reading;
    }
    if (Object.keys(screen).length)
        out.screen_state = screen;
    ...
    if (Object.keys(narrative).length) {
        ...
        out.narrative = { ...Object.fromEntries(Object.entries(narrative).filter(([k]) => !['entries', 'history', 'render_frame'].includes(k))),
            ...
            entries: current.map(e => {
                const entry = { ...obj(e) };
                if (typeof body === 'string' && entry.text === body) {
                    delete entry.text;
                    delete entry.visible_text;
                    entry.event_reading_ref = true;
                }
                else if (entry.text === entry.visible_text)
                    delete entry.visible_text;
                return entry;
            }) };
    }
...
export function section(state, name, offset, limit) {
    ...
    if (name === 'mechanics' && mechanics != null) {
        ...
        for (const key of ['collection', 'combat_collection'])
            if (data[key] != null)
                data[key] = collectionPage(data[key], offset, limit);
        return { ...envelope, data: compact(data) };
    }
    const sources = { deck: g.deck, map: Array.isArray(g.map) ? g.map.map(mapNode) : g.map, map_plan: pick(obj(g.map_plan), [...planKeys, 'route']), piles: pick(c, ['draw_pile', 'discard_pile', 'exhaust_pile', 'draw_pile_order_visible', 'draw_pile_order']),
    ...
    if (Array.isArray(data))
        return { session_id: state.session_id, state_id: state.state_id, section: name, total: data.length, offset, items: compact(data.slice(offset, offset + limit)), next_offset: offset + limit < data.length ? offset + limit : null };
    return { session_id: state.session_id, state_id: state.state_id, section: name, data: compact(data ?? null) };
}
function collectionPage(value, offset, limit) {
