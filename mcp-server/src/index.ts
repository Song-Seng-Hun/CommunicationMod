...
const lifecycle=new Lifecycle(root,link);const metrics=createMetrics(root);
const read={readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false};
const response=(data:Obj)=>({content:[{type:'text' as const,text:JSON.stringify(data)}]});
const receiptView=(value:Obj):Obj=>{
 const receipt=obj(value.receipt),out:Obj={};
 if(Object.keys(receipt).length){
  const r:Obj={};for(const key of ['status','code','message','error','reason'])if(receipt[key]!==undefined)r[key]=receipt[key];
  if(Object.keys(r).length)out.receipt=r;
 }
 if(value.pending===true)out.pending=true;
 if(!out.receipt&&!out.pending)out.status='unknown';
 return out;
};
...
async function guard(tool:string,run:(wait:TimedWait)=>Promise<Obj>,format:ContextFormat='json',lifecycleOnly=false,signal?:AbortSignal){
 const span=metrics.start(tool);let connect_ms=0,wait_ms=0,error_class:MetricsErrorClass='none',connecting=false;let result;
 ...
 }finally{if(span)try{metrics.finish(span,{connect_ms,wait_ms,error_class,response_bytes:Buffer.byteLength(JSON.stringify(result),'utf8')});}catch{}}
 return result!;
}
server.registerTool('sts_game_status',{title:'Downfall status',description:"Test-game process/connection. No launch. No play.",inputSchema:toolSchemas.sts_game_status,annotations:read},()=>guard('sts_game_status',()=>lifecycle.status(),'json',true));
server.registerTool('sts_start_game',{title:'Start Downfall',description:"Start/reuse verified test game only on user launch/play request. Never resumes runs; never chooses actions. Starting: inspect status; no relaunch.",inputSchema:toolSchemas.sts_start_game,annotations:{...read,readOnlyHint:false}},({wait_ms})=>guard('sts_start_game',()=>lifecycle.start(wait_ms),'json',true));
server.registerTool('sts_get_state',{title:'Current decision',description:"Current decision + state-scoped toc. The server pins this state for later context/action calls. Read needed refs via sts_get_context before acting. known_view=previous view_id: short unchanged reply.",inputSchema:toolSchemas.sts_get_state,annotations:read},
 ({wait_ms,view,known_view},extra)=>guard('sts_get_state',async wait=>{
  const state=await wait(()=>link.game.wait(wait_ms,{signal:extra.signal,...(known_view!==undefined?{changed:(candidate:Obj)=>conditionalDecision(candidate,undefined,view).view_id!==known_view}:{})}));
  link.game.present(state);return conditionalDecision(state,known_view,view);
 },'json',false,extra.signal));
server.registerTool('sts_act',{title:'Act + next decision',description:"One offered action from the pinned state. Read needed details first; never auto-discard. Unknown/applied_waiting: inspect request; never replay. Successful calls return the next decision directly.",inputSchema:toolSchemas.sts_act,annotations:{...read,readOnlyHint:false,destructiveHint:true,idempotentHint:false}},
 args=>guard('sts_act',async wait=>{
  ...
  const next=obj(result.state);link.game.present(next);const state=conditionalDecision(next),outcome=String(result.outcome);
  if(outcome==='applied')return {state};
  const data:Obj={outcome,state};if(outcome==='unknown'||outcome==='applied_waiting')data.request_id=result.request_id;
  if(outcome==='rejected'){const r=receiptView(result);if(r.receipt)data.reason=obj(r.receipt).message ?? obj(r.receipt).error ?? obj(r.receipt).reason ?? obj(r.receipt).code ?? obj(r.receipt).status;}
  return data;
 }));
server.registerTool('sts_get_context',{title:'Public context fragments',description:"Read 1-8 refs from the pinned state. Multiple refs share an inline response budget: card refs may return card_summary; read one card ref alone for full detail. Follow next_offset when present. Stale pin: refresh state. Read relevant rules.",inputSchema:toolSchemas.sts_get_context,annotations:read},
...
server.registerTool('sts_get_request',{title:'Action receipt',description:"Inspect an uncertain recent action by request_id without replay. Missing: unknown, not failure; never resend.",inputSchema:toolSchemas.sts_get_request,annotations:read},
 ({request_id})=>guard('sts_get_request',async()=>receiptView(link.game.request(request_id))));
