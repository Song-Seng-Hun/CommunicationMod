...
for(const [id,{capability:c}] of active){for(const s of c.gate.screens??[])index(screens,s,id);for(const r of c.gate.roots??[])index(rootsIndex,r,id);for(const a of c.gate.actions??[])index(actionsIndex,a,id);if(c.gate.special)index(specials,c.gate.special,id);}
