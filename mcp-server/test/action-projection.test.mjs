...
import {publicParameterSchema,assertPublicArguments,injectFixedArguments} from '../dist/action-projection.js';
...
test('required pinned const and one-value enum parameters stay server-side',()=>{
 ...
 assert.deepEqual(assertPublicArguments(schema,{nodes:['1,2'],commentary:'설명'}),{nodes:['1,2'],commentary:'설명'});
 assert.throws(()=>assertPublicArguments(schema,{nodes:['1,2']}),/Missing required action arguments: commentary/);
 assert.throws(()=>assertPublicArguments(schema,{nodes:['1,2'],commentary:'설명',reading_id:'hidden'}),/Unexpected action argument: reading_id/);
...
test('fully pinned required parameter objects disappear from public schema',()=>{
 ...
 assert.deepEqual(assertPublicArguments(schema,{}),{});
...
test('optional const parameters stay public and are never silently injected',()=>{
 ...
 assert.deepEqual(assertPublicArguments(schema,{value:3}),{value:3});
