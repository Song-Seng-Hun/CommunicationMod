...
test('agent skill references actual event field and deterministic batch/format rules',()=>{
 ...
 for(const term of ['parameters_ref','parameters:{}','refs','offset','compact','JSON','sts_get_request','unknown','applied_waiting'])assert.ok(text.includes(term),term);
});
...
test('skill routes optional examples and separates templates from live game facts',()=>{
 for(const term of ['guidance','revision','bindings','sts_game_status({})','sts_start_game({wait_ms:15000})','not permissions','Do not send $bind'])assert.ok(text.includes(term),term);
});
