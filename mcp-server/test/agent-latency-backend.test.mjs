...
test('isolated evaluation runs pinned action/state guards, never game lifecycle',async()=>{
 const {client,call,log}=await connect(1);try{assert.equal((await client.listTools()).tools.length,6);const shown=await call('sts_get_state'),alias=shown.actions[0].id;assert.match(alias,/^a\d+$/);const result=await call('sts_act',{action_id:alias});assert.equal(result.outcome,undefined);assert.equal(result.state.screen,'VICTORY');const duplicate=await client.callTool({name:'sts_act',arguments:{action_id:alias}});assert.equal(duplicate.isError,true);}finally{await client.close();}
...
for(let id=2;id<=6;id++)test(`scenario ${id}: required facts and safety reachable through actual MCP`,async()=>{
 const {client,call,log}=await connect(id),spec=scenario(id);try{
  ...
  const shown=await call('sts_get_state');const context=(refs,offset=0)=>call('sts_get_context',{refs,offset});
  if(id===2)await context(['hand/0','actions/0/parameters']);if(id===3)await context(['screen/cards/0','reward_navigation','potion_controls']);if(id===4){const schema=await context(['actions/0/parameters']);assert.equal(JSON.stringify(schema).includes('reading-tower-10'),false);let offset=0;do{const r=await context(['screen/event_reading/body_text'],offset);offset=r.fragments[0].next_offset??null;}while(offset!==null);}if(id===5){await context(['collection/cards'],20);await context(['collection/cards/32']);}
  if(spec.act){const alias=shown.actions[0].id;assert.match(alias,/^a\d+$/);const args=id===2?{target_index:0}:id===4?{commentary:'낡은 탑 입구 17. 붉은 문 체력 9 감소. 푸른 문 보상 없이 떠남.'}:{};const result=await call('sts_act',{action_id:alias,arguments:args});assert.equal(result.outcome,id===6?'unknown':undefined);if(id===6){assert.ok(result.request_id);const receipt=await call('sts_get_request',{request_id:result.request_id});assert.equal(receipt.pending,true);await call('sts_get_state');}}
